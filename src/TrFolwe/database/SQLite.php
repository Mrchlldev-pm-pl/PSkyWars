...
class SQLite
{
     ...
     * @return void
     */
    public function updateKillData(string $playerName, int $kill): void
    {
        $data = $this->sql->prepare("UPDATE playerTable SET kill = :kill WHERE playerName = :playerName");
        $data->bindValue("kill", $kill);
        $data->bindValue("playerName", $playerName);
        $data->execute();
    }
     ...
     * @return void
     */
    public function updateWinData(string $playerName, int $winCount) :void
    {
        $data = $this->sql->prepare("UPDATE playerTable SET gameWin = :winCount WHERE playerName = :playerName");
        $data->bindValue("winCount", $winCount);
        $data->bindValue("playerName", $playerName);
        $data->execute();
    }
